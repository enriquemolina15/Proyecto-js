...
function mostrarDatos() {
  ...
  cuerpo.innerHTML = "";
  const claves = Object.keys(sessionStorage); // ← obtenemos todas las claves

  
  if (claves.length > 0) {
    claves.forEach(function (clave) {
      if (clave === "IsThisFirstTime_Log_From_LiveServer") return;
      const valor = sessionStorage.getItem(clave); // ← obtenemos su valor
